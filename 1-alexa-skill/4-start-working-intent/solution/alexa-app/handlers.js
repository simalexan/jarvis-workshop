...
const handlers = {
  ...
  'ActivateApp': function() {
    const intentObj = this.event.request.intent;
    if (!intentObj.slots.appName.value) {
      this.response.speak("App name is missing");
    } else {
      this.response.speak(`Activating application number: ${intentObj.slots.appName.value}!`);
    }
    this.emit(":responseReady");
  }
};
